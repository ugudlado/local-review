...
function resolveSpecPath(featureId, repoRoot) {
...
export function createSpecRoute(repoRoot) {
    const app = new Hono();
    ...
    app.get("/:id/diagrams", async (c) => {
        ...
        const diagramsDir = path.join(wtPath, "specs", "active", featureId, "diagrams");
        let diagrams = [];
        try {
    ...
    app.get("/:id/diagrams/:name", async (c) => {
        ...
        const diagramFilePath = path.join(wtPath, "specs", "active", featureId, "diagrams", diagramName);
        try {
