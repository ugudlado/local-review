...
function filterActiveBranches(state) {
    const localBranches = state?.localBranches ?? [];
    const unmergedBranches = new Set(state?.unmergedBranches ?? []);
    const worktreeBranches = new Set((state?.worktrees ?? []).map((wt) => wt.branch));
    const archivedIds = state?.archivedFeatureIds ?? new Set();
    return localBranches.filter((b) => {
        if (b === "main" || b === "master")
            return true;
        const slug = b.includes("/") ? b.slice(b.lastIndexOf("/") + 1) : b;
        if (archivedIds.has(slug) && !worktreeBranches.has(b))
            return false;
...
function resolveWorktree(requestedPath, repoRoot) {
...
async function buildDiffBundle(worktreePath, requestedTarget, requestedSource, localBranches, currentBranch) {
    ...
    const sourceBranch = resolveSourceBranch(requestedSource, currentBranch, localBranches);
    const committedDiff = await execGit(["diff", "--no-color", `${targetBranch}...${sourceBranch}`], worktreePath);
    let uncommittedDiff = "";
    if (sourceBranch === currentBranch) {
        const trackedDiff = await execGit(["diff", "--no-color", "HEAD"], worktreePath);
        const untrackedOutput = await execGit(["ls-files", "--others", "--exclude-standard"], worktreePath);
        const untrackedFiles = untrackedOutput.split("\n").filter(Boolean);
        const untrackedDiffs = await Promise.all(untrackedFiles.map(async (file) => {
            try {
                const { stdout } = await execFileAsync("git", ["diff", "--no-color", "--no-index", "/dev/null", file], { cwd: worktreePath, maxBuffer: 20 * 1024 * 1024 }).catch((err) => err.code === 1 && err.stdout
                    ? { stdout: err.stdout }
                    : Promise.reject(err));
                return stdout;
            }
    ...
    if (!uncommittedDiff || !committedDiff) {
        ...
        const committedFiles = new Set(committedDiff
            .split("\n")
...
// Route factory
// ---------------------------------------------------------------------------
export function createContextRoute(repoRoot) {
    const app = new Hono();
    // GET /context
    app.get("/context", async (c) => {
        ...
        const defaultTargetBranch = resolveTargetBranch(requestedTarget, localBranches);
    ...
    app.get("/diff", async (c) => {
        ...
        const localBranches = state?.localBranches ?? [];
        const rawWorktrees = state?.worktrees ?? [];
        const effective = resolveEffectiveWorktree(selectedWorktree, requestedSource, rawWorktrees);
        try {
            const bundle = await buildDiffBundle(effective.path, requestedTarget, requestedSource, localBranches, effective.branch);
    ...
    app.get("/commits", async (c) => {
        ...
        const effective = resolveEffectiveWorktree(selectedWorktree, requestedSource, rawWorktrees);
        try {
            const result = await buildCommitList(effective.path, requestedTarget, requestedSource, localBranches, effective.branch);
    ...
    app.get("/commit-diff", async (c) => {
        ...
        try {
            const diff = await execGit(["show", "--no-color", hash], selectedWorktree.path);
            return c.json({ diff });
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
