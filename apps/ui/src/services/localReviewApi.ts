...
export const localReviewApi = {
  ...
  async getDiffBundle(params: { worktreePath?: string; sourceBranch?: string; targetBranch?: string }): Promise<DiffBundle> {
  ...
  async getCommits(params: { worktreePath?: string; sourceBranch?: string; targetBranch?: string }): Promise<CommitInfo[]> {
    const query = new URLSearchParams();
    if (params.worktreePath) {
      query.set('worktree', params.worktreePath);
    }
  ...
  async getCommitDiff(params: { worktreePath?: string; commit: string }): Promise<string> {
    const query = new URLSearchParams();
    if (params.worktreePath) {
      query.set('worktree', params.worktreePath);
    }
  ...
  async listSessions(): Promise<string[]> {
    const response = await fetch('/local-api/sessions');
    const data = await parseJson<{ sessions: string[] }>(response);
    return data.sessions;
  },

  async getSession(fileName: string): Promise<ReviewSession> {
    const response = await fetch(`/local-api/sessions/${encodeURIComponent(fileName)}`);
    const data = await parseJson<{ session: ReviewSession }>(response);
    return data.session;
  },

  async saveSession(session: ReviewSession): Promise<{ fileName: string; path: string }> {
    const response = await fetch('/local-api/sessions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(session),
    });
    return await parseJson<{ fileName: string; path: string }>(response);
  },
};
