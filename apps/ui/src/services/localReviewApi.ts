...
export const localReviewApi = {
  async getContext(
    ...
    const url = withRepo(`/api/context${suffix}`, repo ?? null);
  ...
  async getDiffBundle(params: {
    ...
    const url = withRepo(`/api/diff${suffix}`, params.repo ?? null);
    const response = await fetch(url);
  ...
  async getCommits(params: {
    ...
    const suffix = query.toString() ? `?${query.toString()}` : "";
    const url = withRepo(`/api/commits${suffix}`, params.repo ?? null);
    const response = await fetch(url);
  ...
  async getFileContent(params: {
    ...
    const url = withRepo(`/api/file?${query.toString()}`, params.repo ?? null);
    const response = await fetch(url);
