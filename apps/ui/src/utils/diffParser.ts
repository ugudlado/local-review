...
function parseDiffGitLine(line: string): { oldPath: string; newPath: string; newPrefix: string } | null {
...
function pickDisplayPath(oldPath: string, newPath: string, status: DiffFileStatus): string {
...
export function parseUnifiedDiff(diff: string): DiffFile[] {
  ...
  const ensureFile = () => {
    ...
    if (!displayPath && pendingRawGitLine) {
      ...
      if (idx !== -1) displayPath = pendingRawGitLine.slice(idx + sep.length).replace(/[\r\t ]+$/, '');
    }
  ...
  return files.filter((file) => file.hunks.length > 0 && file.path.trim().length > 0);
}
