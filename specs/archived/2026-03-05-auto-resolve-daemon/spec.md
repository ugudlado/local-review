...
- [x] Cold-start a Claude CLI session when the Vite dev server starts, capturing the session ID
- [x] Automatically trigger thread resolution when verdict changes to `changes_requested`
- [x] Resume the existing Claude session (via `--resume <session_id>`) for each resolve request
- [x] Support both code and spec session types
- [x] Update resolved threads via existing PATCH API endpoints
- [x] Show resolving status in the UI (spinner + completion summary)
- [x] Serialize resolve requests (prevent concurrent runs)

### Nice to Have

- [x] Manual "Resolve with Claude" button for re-triggering without changing verdict
- [x] Per-feature session IDs (currently one global daemon)
...
- [x] Vite server cold-starts a Claude session on boot without blocking server startup
- [x] Clicking "Request Changes" with open threads triggers automatic resolution
- [x] Resolved threads appear in the UI without manual refresh (WebSocket push)
- [x] Concurrent resolve requests are serialized (no race conditions)
- [x] Works for both code review and spec review sessions
- [x] UI shows resolving spinner and completion summary
